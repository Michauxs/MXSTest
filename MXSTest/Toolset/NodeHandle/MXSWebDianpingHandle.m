...
@implementation MXSWebDianpingHandle
	...
	for (HTMLNode *divNode in divNodes) {
		...
		for (HTMLNode *spanNode in spanNodes) {
			if ([[spanNode getAttributeNamed:@"itemprop"] isEqualToString:@"street-address"]) {
				NSString *addr = [NodeHandle extractionStringFromString:[spanNode rawContents]];
				NSLog(@"addr:%@", addr);
		...
		for (HTMLNode *ddNode in ddNodes) {
			if ([[ddNode getAttributeNamed:@"itemprop"] isEqualToString:@"tel"]) {
				NSString *phone = [NodeHandle extractionStringFromString:[ddNode rawContents]];
				NSLog(@"phon:%@", phone);
