...
#import "MXSAudioPlay.h"

static UInt32 gBufferSizeBytes = 0x10000;//It muse be pow(2,x)

@implementation MXSAudioPlay
@synthesize queue;

//回调函数(Callback)的实现
static void BufferCallback(void *inUserData,AudioQueueRef inAQ, AudioQueueBufferRef buffer){
	MXSAudioPlay* player = (__bridge MXSAudioPlay*)inUserData;
...
//	status = AudioFileReadPackets(audioFile, NO, &numBytes, packetDescs, packetIndex,&numPackets, audioQueueBuffer->mAudioData);
	status = AudioFileReadPacketData(audioFile, NO, &numBytes, packetDescs, packetIndex,&numPackets, audioQueueBuffer->mAudioData);
...
//	AudioFileReadPackets(audioFile, NO, &numBytes, packetDescs, packetIndex, &numPackets, buffer->mAudioData);
	AudioFileReadPacketData(audioFile, NO, &numBytes, packetDescs, packetIndex, &numPackets, buffer->mAudioData);
	if(numPackets >0){
