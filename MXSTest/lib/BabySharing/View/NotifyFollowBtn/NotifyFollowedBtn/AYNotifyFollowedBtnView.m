...
#import "AYNotifyFollowedBtnView.h"
...
@implementation AYNotifyFollowedBtnView
- (void)postPerform {
    [super postPerform];
    [self setBackgroundImage:PNGRESOURCE(@"command_follow") forState:UIControlStateNormal];
}
...
- (void)selfClicked {
    ...
    [cmd performWithResult:[dic copy] andFinishBlack:^(BOOL success, NSDictionary * result) {
        if (success) {
            ...
            [[[UIAlertView alloc]initWithTitle:@"取消关注失败" message:@"请检查网络连接是否正常" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil]show];
        }
    }];
}
@end
